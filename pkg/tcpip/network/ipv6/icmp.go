...
func (e *endpoint) handleICMP(r *stack.Route, pkt *stack.PacketBuffer, hasFragmentHeader bool) {
	...
	case header.ICMPv6NeighborSolicit:
		...
		// to avoid a race condition where the neighbor entry is removed from when
		// it got updated to when the packet is sent. This is okay because the table
		// would have gotten updated with the link address we will resolve the route
		// with.
		//
		// Note, if the source link address is unspecified and the this is a unicast
		// solicitation, we may need to perform neighbor discovery to send the
		// neighbor advertisement response. This is also expected as per RFC 4861
		// section 7.2.4:
		//
		...
		na.SetSolicitedFlag(r.RemoteAddress != header.IPv6AllNodesMulticastAddress)
