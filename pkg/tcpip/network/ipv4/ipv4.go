...
func (e *endpoint) HandlePacket(r *stack.Route, pkt *stack.PacketBuffer) {
	...
	if len(h) > header.IPv4MinimumSize {
		aux, _, err := processIPOptions(r, h.Options(), header.IPv4OptionUsageReceive)
		if err != nil {
			// XXX use if errors.Is(err, os.ErrExist)
			switch err {
			case
				header.ErrIPv4OptDuplicate,
				header.ErrIPv4RecordRouteOptInvalidPointer,
				header.ErrIPv4TimestampOptInvalidPointer,
				header.ErrIPv4TimestampOptOverflow:
...
func handleTimestamp(tsOpt header.IPv4OptionTimestamp, localAddress tcpip.Address, op header.IPv4OptionsUsage) (byte, error) {
	flags := tsOpt.Flags()
	optlen := tsOpt.Size()
	...
	if op == header.IPv4OptionUsageReceive {
		return 0, nil
	}
...
// without timestamps.
func handleRecordRoute(tsOpt header.IPv4OptionRecordRoute, localAddress tcpip.Address, op header.IPv4OptionsUsage) (byte, error) {
	optlen := tsOpt.Size()

	if optlen > header.IPv4OptionRecordRouteMaxSize ||
		optlen < header.IPv4AddressSize+header.IPv4OptionRecordRouteHdrLength {
		return 1, header.ErrIPv4RecordRouteOptInvalidLength
	}
	...
	//
	// Also make optlen based on the data part only.
	pointer := start - (header.IPv4OptionRecordRouteHdrLength + 1)
	optlen -= header.IPv4OptionRecordRouteHdrLength

	if pointer == optlen {
		return 0, nil
	}

	if pointer > optlen-header.IPv4AddressSize {
		// The data area isn't full but there isn't room for a new entry.
		// RFC 791 (see above) suggests this is a different error to being full
		// as we don't use the overflow counter.
		return 2, header.ErrIPv4RecordRouteOptInvalidPointer
	}

	target := header.IPv4OptRecordRouteEntry(tsOpt[start-1 : start+header.IPv4AddressSize-1])

	target.SetIPAddress(localAddress)
	tsOpt.IncPointer(header.IPv4AddressSize)
	return 0, nil
}
...
//
func processIPOptions(r *stack.Route, orig header.IPv4OptionsBuffer, op header.IPv4OptionsUsage) (byte, header.IPv4OptionsBuffer, error) {
	...
	for {
		...
		if optType == header.IPv4OptionListEndType {
			optIter.PushByte(optType)
			optIter.CloseBuffer()
			return 0 /*errCursor */, optIter.NewOptions(), nil /* err */
		}
		...
		case header.IPv4OptionTimestamp:
			...
			switch op {
			case header.IPv4OptionUsageFragN:
				continue
			}
		...
		case header.IPv4OptionRecordRoute:
			...
			newBuffer := optIter.WriteBuffer()[:len(option)]
			copy(newBuffer, option.Contents()) // Prepare for in-place processing.
			offset, ret := handleRecordRoute(header.IPv4OptionRecordRoute(newBuffer), r.LocalAddress, op)
			if ret != nil {
				return optIter.ErrCursor + offset, nil, ret
			}
			optIter.ConsumeBuffer(int(option.Size()))
		...
		default:
			...
			// This is a unknown option to us. Depending on how we are being called,
			// we need to do different things with it.
			switch op {
			case header.IPv4OptionUsageEcho:
				// leave it out of the reply
				continue
			case header.IPv4OptionUsageFragN:
				// Only options with the top bit set are copied into non initial
				// fragments.  RFC 791 page 15:
				//
				...
				fallthrough
			default:
				// if forwarding or creating the first fragment, give a copy of
				// everything.
				newBuffer := optIter.WriteBuffer()[:option.Size()]
				copy(newBuffer, option.Contents())
				optIter.ConsumeBuffer(int(option.Size()))
			}
		}
	}
