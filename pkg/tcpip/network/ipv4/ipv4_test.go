...
func compareFragments(packets []*stack.PacketBuffer, sourcePacket *stack.PacketBuffer, mtu uint32, wantFragments []fragmentInfo, proto tcpip.TransportProtocolNumber) error {
	...
	if diff := cmp.Diff(reassembledPayload.ToView(), expected); diff != "" {
...
func TestFragmentationErrors(t *testing.T) {
	...
	for _, ft := range tests {
		t.Run(ft.description, func(t *testing.T) {
			pkt := testutil.MakeRandPkt(ft.transHdrLen, header.IPv4MinimumSize, []int{ft.payloadSize}, header.IPv4ProtocolNumber)
